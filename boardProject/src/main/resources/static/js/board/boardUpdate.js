...
// * Set을 사용하는 이유 : X 버튼이 눌러질 때 order가 저장될 예정인데
...
// X 버튼들
const deletImageList = document.getElementsByClassName("delete-image");
...
for(let i = 0; i < inputImageList.length ; i++ ){
  ...
  deletImageList[i].addEventListener("click", () => {
    ...
    // -> 기존에 이미지가 있었는데 i번째 이미지 X 버튼 눌러서 삭제함 --> DELETE 수행
...
form.addEventListener("submit", e => {
  ...
  // 제출 전에 form 태그 마지막 자식으로 input 추가 한 후 제출
  // -> 해당 input에는 삭제된 이미지 순서(deleteOrderList)를 담음
