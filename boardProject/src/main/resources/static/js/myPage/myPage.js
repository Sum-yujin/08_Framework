...
changePw?.addEventListener("submit", e => {
  ...
  */
  ...
  if ( !(lengthCheck && letterCheck && numberCheck && specialCharCheck) ){
    alert("영어,숫자,특수문자 포함하여, 6~20자 사이로 작성 바랍니다")
    e.preventDefault();
    return;
  }
...
// 프로필 이미지 업로드 상태에 따라서 상태인지 구분하는 값
// -1 : 프로필 이미지를 바꾼적이 없음(초기상태)
//  0 : 프로필 이미지 삭제 (X버튼 클릭)
//  1 : 새 이미지 선택
let statusCheck = -1;

let lastValidFiles = null; // 마지막으로 선택된 파일을 저장할 변수
...
if(imageInput != null) { // 프로필 변경 화면인 경우
 ...
 const updatePreview = (file) => {
    ...
    lastValidFiles = file; // 선택된 파일을 lastValidFiles에 대입(복사)
  ...
  imageInput.addEventListener("change", e => {
    ...
    if(file === undefined){
      ...
      /* 이전 선택한 파일 유지하는 코드 */
      // -> 이전 선택한 파일을 저장할 전역 변수(lastValidFiles) 선언

      // 이전에 선택한 파일이 없는 경우
      // == 현재 페이지 들어와서 프로필 이미지 바꾼적이 없는 경우
      if(lastValidFiles === null) return;
      ...
      // input의 files 변수에 lastValidFiles이 추가된 files 대입
      imageInput.files = dataTransfer.files;

      // 이전 선택된 파일로 미리보기 되돌리기
      updatePreview(lastValidFiles);
